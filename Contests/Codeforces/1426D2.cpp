...
int main(){
    ...
    w(t){
        ...
        h.build(v);
        Hash h1(sz(str), 997, ll(999727999));
        h1.build(v);
        int a = -1, b = sz(str) / 2, mid;
        while(b - a > 1){
            mid = (a + b) >> 1;
            if(h.getLR(0, mid) == h.getRL(sz(str) - mid - 1, sz(str) - 1) && h1.getLR(0, mid) == h1.getRL(sz(str) - mid - 1, sz(str) - 1))
                a = mid;
            else
                b = mid;
        }
        ...
        int ml = -1, mr = -1;
        fore(i, L + 1, R){
            if(h.getLR(L + 1, i) == h.getRL(L + 1, i) && h1.getLR(L + 1, i) == h1.getRL(L + 1, i)){
                mr = i; 
                ml = L + 1;  
                mx = mr - ml + 1;     
            }
        ...
        for(int i = R - 1; i >= L + 1; i--){
            if(h.getLR(i, R - 1) == h.getRL(i, R - 1) && h1.getLR(i, R - 1) == h1.getRL(i, R - 1)){
                if(R - 1 - i + 1 > mx){
                    ml = i;
                    mr = R - 1;
                    mx = R - 1 - i + 1;
