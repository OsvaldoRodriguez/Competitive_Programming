...
void solve(){
    ...
    // for(int i : p) cout << i << " " << s.substr(i, p.size() - i) << "\n";
    ...
    while(query--){
        cin >> t;
        int a = 0, b = p.size(), mid;
        while(b - a > 1){
            mid = (a + b) >> 1;
            string aux = s.substr(p[mid], min(t.size(), p.size() - p[mid]));
            if(aux.size() < t.size()){
                string add(t.size() - aux.size(), '$');
                aux += add;
            }
            if(aux <= t)
                a = mid;
        ...
        int L = 10, R = -1;
        string cur = s.substr(p[a], t.size());
        if(cur == t)
            R = a;
        ...
        string cur2 = s.substr(p[b], t.size());
        if(cur2 == t)
            L = b;
